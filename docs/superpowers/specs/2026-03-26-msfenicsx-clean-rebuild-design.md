...
> 2026-03-27 addendum: `evaluation/` and `optimizers/` have now moved onto a multicase multiobjective mainline. The core canonical objects remain `scenario_template`, `thermal_case`, and `thermal_solution`; objective and constraint definitions live in standalone `evaluation_spec` files, while design variables and optimizer settings live in standalone `optimization_spec` files. The active optimizer path is hot/cold multicase evaluation plus Pareto search rather than the earlier constrained single-objective baseline. See `docs/reports/R63_msfenicsx_multicase_multiobjective_reset_20260327.md` and `docs/reports/R64_msfenicsx_paper_grade_b0_rollout_20260327.md`.
...
- [R60](./docs/reports/R60_msfenicsx_2d_fenicsx_migration_initial_report_20260326.md) remains the top-level migration and research-direction reference
...
This design aligns with the migration logic in [R60](./docs/reports/R60_msfenicsx_2d_fenicsx_migration_initial_report_20260326.md):
