...
| R5. `.worktree/logging-viz-refactor/` modifies `analytics/pareto.py`.            | Merge conflicts with comparison layer.                                 | Keep comparison self-contained on `pymoo.indicators.hv`; once the refactor lands, re-point the comparison layer to its analytics API without changing the artifact contract. |
