...
`benchmark_seed` 控制 scenario template 的参数采样和布局放置，用来生成不同的 concrete thermal case。`algorithm.seed` 控制固定 case 上的 optimizer 随机性，通过 pymoo 的 `seed=` 传入，并进一步变成 `algorithm.random_state`。

因此每个 scenario-method cell 包含：

```text
3 benchmark instances × 3 optimizer replicates = 9 runs
```

matrix runner 不能只依赖当前 `run-benchmark-suite` 的 seed 行为，因为该命令目前只暴露 `--benchmark-seed`。每个 leaf run 必须使用冻结的临时 optimization-spec snapshot，并在 snapshot 中写入选定的 `benchmark_source.seed`、`algorithm.seed`、population size、generation count、backbone/mode 和 LLM profile。
...
```text
3 scenarios × 3 raw backbones × 3 benchmark seeds × 3 algorithm seeds = 81 runs
```
...
```text
3 scenarios × 1 union mode × 3 benchmark seeds × 3 algorithm seeds = 27 runs
```
...
```text
3 scenarios × 6 LLM profiles × 3 benchmark seeds × 3 algorithm seeds = 162 runs
```
...
M1_raw_backbone_512eval           81 runs
M2_nsga2_union_512eval            27 runs
M3a_llm_gpt_5_4_512eval           27 runs
M3b_llm_qwen3_6_plus_512eval      27 runs
M3c_llm_glm_5_512eval             27 runs
M3d_llm_minimax_m2_5_512eval      27 runs
M3e_llm_deepseek_v4_flash_512eval 27 runs
M3f_llm_gemma4_512eval            27 runs
...
每个 scenario-method/profile cell 在 9 runs 上报告：
...
由于这 9 个样本具有嵌套结构，方法比较时应先在每个 benchmark seed 内聚合 optimizer replicates，再计算 paired differences。报告应强调 effect size、paired differences、win counts、average ranks 和 uncertainty intervals，而不是主要依赖 p-value，因为 benchmark instances 只有 3 个。
...
1. 每个 primary outcome 的 distribution plots，每个 scenario-method/profile cell 显示 9 个 run points。
2. Paired-difference plots：在 benchmark seed 内聚合后比较 union 和 LLM 相对 raw 的差异。
...
8. 增加 focused tests，覆盖 spec generation、seed snapshotting、run-index creation、retry behavior 和 aggregation formulas。
